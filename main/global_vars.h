...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
//Флаги светодиодов
extern bool green_led_frame_change;					//Светодиод, который зажигается при работе привода ЛПМ
extern bool green_led_sublight_en;					//Лампочка показывающая что подсветка работает
extern bool yellow_led_feed_coil;						//Светодиод показывающий работу раздающей бобины
extern bool yellow_led_take_coil;						//То же самое, но с принимающей бобины
extern bool red_led_alarm;									//Лампочка тревоги, которая зажигается если возникают какие-то проблемы в работе сканера
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
extern bool led_enabled;										//Вкл/выкл подсветки кадра - защита красителей плёнки. Включается автоматически при выходе из паузы
//extern bool led_enabled_lock;								//Флаг защиты от повторного срабатывания
extern bool service_lane_lock;							//Флаг отключающий первый слева вертикальный ряд кнопок по время процесса сканирования
extern bool engines_enabled;								//Предотвращает преждевременный запуск двигателей, например на этапе настройки подсветки
extern bool camera_shot;										//Сигнал на затвор камеры
extern bool camera_busy;										//Флажок "камера занята"
//extern bool scan_enabled;										//Флаг запущенного сканирования
extern bool high_speed;											//Флажок переключения низкой и высокой скорости
extern bool take_coil_freeze;								//Флажок отключения двигателя принимающей бобины
extern bool take_coil_freeze_lock;					//Защита от повторного срабатывания
extern bool drop_detect;										//Бит срабатывания датчика выпадения плёни
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
extern uint16_t callback_sensor;
extern uint16_t led_calibration;
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
extern uint8_t def;														//Защитный интервал, который предотвращает мгновенное срабатывание кнопки остановки
extern uint16_t xt_shutter_preset[14];				//Пресеты задержек для затвора фуджи xt4
extern uint8_t xt_shutter_mode;								//Выбранная выдержка затвора
extern uint16_t xt_shutter_delay;							//Задержка на время работы затвора
