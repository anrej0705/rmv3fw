...
inline void check_buttons()
{
	...
	switch(ui_code)
	{
		...
		case 42:
		{
			...
			if(key_code == 15)
			{
				ui_code = 3;		//Здесь должен быть переход на жёлтый, но его пока нет. Возвращаемся в паузу
		...
		case 80:
		{
			//Настройка задержки во время работы затвора
			switch(key_code)
			{
				case 5:
				{
					...
					//Защита от выхода за пределы массива назад
					if(xt_shutter_mode >= 14)
					{
						xt_shutter_mode = 13;
					}
				...
				case 7:
				{
					...
					//Защита от выхода за пределы массива вперёд
					if(xt_shutter_mode >= 14)
					{
						xt_shutter_mode = 0;
					}
				...
				case 15:
				{
					//Устанавливаем задержку протяжки плёнки на время работы затвора
					xt_shutter_delay = xt_shutter_preset[xt_shutter_mode];
					
					//Переключаемся на экран настройки баланса белого
					ui_code = 1;
	...
	if((GPIOB->IDR >> 7 & 0x0001) == 1 && !key_start_lock)
	{
		key_start_lock = 1;
		key_code = 20;
	}
	else if((GPIOB->IDR >> 7 & 0x0000) == 0 && key_start_lock)
	{
		key_start_lock = 0;
	}
	
	//Переключаем подсветку
	if(key_code == 12 && !service_lane_lock)
	{
